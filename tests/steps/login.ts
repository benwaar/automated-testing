...
When("I enter the password as {string}", async function (this: CustomWorld, password: string) {
  ...
  const passwordField = this.page!.locator('input[name="password"], input[id="password"], input[type="password"]').first();
...
When("I click on login button", async function (this: CustomWorld) {
  ...
  const loginButton = this.page!.locator('input[type="submit"], button[type="submit"], button:has-text("Sign In"), button:has-text("Log In"), #kc-login').first();
...
Then("User should logged in successfully", async function (this: CustomWorld) {
  ...
  try {
    ...
    const adminElements = this.page!.locator('.pf-c-page, .keycloak-admin, [data-testid="admin-console"], .navbar, .pf-c-nav').first();
...
Then("I should see an error message", async function (this: CustomWorld) {
  ...
  const errorMessage = this.page!.locator('#input-error, .pf-c-alert, .alert-error, [class*="error"], [role="alert"], .kc-feedback-text').first();
...
Then("Logout from the application", async function (this: CustomWorld) {
  try {
    ...
    const userMenu = this.page!.locator('.pf-c-dropdown__toggle, .user-menu, [data-testid="user-dropdown"], button[aria-label*="user"]').first();
    await userMenu.waitFor({ state: 'visible', timeout: 10000 });
    await userMenu.click();
    
    // Look for sign out/logout option in the dropdown
    const logoutButton = this.page!.locator('text=/sign out/i, text=/logout/i, [href*="logout"], a:has-text("Sign out")').first();
    ...
    await this.page!.waitForURL(url => {
      const urlStr = url.toString();
      return urlStr.includes('/protocol/openid-connect/auth') || urlStr.includes('/realms/master/protocol/openid-connect/logout');
    }, { timeout: 10000 });
